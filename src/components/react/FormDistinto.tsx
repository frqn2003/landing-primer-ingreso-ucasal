...
export default function FormDistinto({ codcarInicial, onSubPage }: { codcarInicial?: string, onSubPage?: boolean }) {
    ...
    return (
        <form ref={setFormRef} role="form" id="pedidoinfo_distinto" method="post" encType="multipart/form-data" action="/postulantes_mail1.php"
            autoComplete="on"
            onSubmit={handleSubmit(
                async () => {
                    ...
                    (document.getElementById('g-recaptcha-response-distinto') as HTMLInputElement).value = recaptchaToken

                    const formEl = document.getElementById('pedidoinfo_distinto') as HTMLFormElement
                    const formData = new FormData(formEl)

                    await fetch('/postulantes_mail1.php', { method: 'POST', body: formData })

                        ; (window as any).dataLayer?.push({ event: 'form_enviado_pedidoinfo', form_id: 'pedidoinfo_distinto' })
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className={`rounded-lg border ${!carreraCompleta ? 'bg-gray-100 border-gray-200 pointer-events-none' : `bg-white ${claseBorde(carreraCompleta, !!nombre && !errors.nombre)}`}`}>
                        <input type="text" {...register("nombre")} id="nombre"
                            ...
                            aria-describedby={errors.nombre ? 'error-nombre-distinto' : undefined}
                        />
                    </div>
                    <div className={`rounded-lg border ${!carreraCompleta ? 'bg-gray-100 border-gray-200 pointer-events-none' : `bg-white ${claseBorde(carreraCompleta, !!email && !errors.email)}`}`}>
                        <input type="email" id="email"
                            ...
                            aria-describedby={errors.email ? 'error-email-distinto' : undefined}
                            {...register('email')} />
                    </div>
                </div>
                {(errors.nombre || errors.email) && (
                    <div className="flex flex-col gap-0.5 mt-1">
                        {errors.nombre && <p id="error-nombre-distinto" className="text-red-500 text-xs" role="alert">{errors.nombre.message}</p>}
                        {errors.email && <p id="error-email-distinto" className="text-red-500 text-xs" role="alert">{errors.email.message}</p>}
                    </div>
                )}
                ...
                <div className={`flex flex-row gap-2 mt-3 ${!carreraCompleta ? 'pointer-events-none opacity-75' : ''}`}>
                    ...
                    <div className={`rounded-lg border w-16 ${!carreraCompleta ? 'bg-gray-100 border-gray-200' : `bg-white ${claseBorde(carreraCompleta, !!codArea && !errors.cod_area)}`}`}>
                        <input type="tel" id="cod" size={4} maxLength={4} pattern="[0-9]*" inputMode="numeric"
                            ...
                            aria-describedby={errors.cod_area ? 'error-cod_area-distinto' : undefined}
                            {...register('cod_area')} />
                    </div>
                    ...
                    <div className={`rounded-lg border flex-1 ${!carreraCompleta ? 'bg-gray-100 border-gray-200' : `bg-white ${claseBorde(carreraCompleta, !!tel && !errors.tel)}`}`}>
                        <input type="tel" id="tel" size={8} maxLength={8} inputMode="numeric" pattern="[0-9]+"
                            ...
                            aria-describedby={errors.tel ? 'error-tel-distinto' : undefined}
                            {...register('tel')} />
                    </div>
                </div>
                {(errors.cod_area || errors.tel) && (
                    <div className="flex flex-col gap-0.5 mt-1">
                        {errors.cod_area && <p id="error-cod_area-distinto" className="text-red-500 text-xs" role="alert">{errors.cod_area.message}</p>}
                        {errors.tel && <p id="error-tel-distinto" className="text-red-500 text-xs" role="alert">{errors.tel.message}</p>}
                    </div>
                )}
            </div>
            ...
            <input type="hidden" id="g-recaptcha-response-distinto" name="g-recaptcha-response" />
            <input type="hidden" id="fecha_formulario" name="fecha_formulario" />
            <div className="flex justify-center mt-4">
                <button id="formButtonDistinto" type="submit"
                    disabled={!todosCompletos}
                    style={enviando ? { display: 'none' } : undefined}
                    className={`w-full font-bold text-sm px-5 py-3 text-center text-white rounded-lg transition-colors duration-200 ease-in-out ${todosCompletos ? 'cursor-pointer bg-gradient-to-b from-(--rojo-ucasal) to-(--rojo-dark-ucasal) hover:opacity-90' : 'cursor-not-allowed bg-gray-300'}`}
                >
                    Quiero que me contacten
                </button>
                <div id="spinnerContainerDistinto" className={enviando ? '' : 'hidden'} role="status">
